...
Type decTypeCheck(AST* ast)
{
  ...
  else if(ast->astoken->sym == NTS_CMPD_STMT)
  {
    ...
    for(i = ast->operandNum - 1; i >= 0; i--)//might be good to check this backwards actually
    {
      ...
      if(s == NTS_SEL_STMT && (t == RETINT_ELSE || t == RETVOID_ELSE))
      {
        t = t - (RETINT_ELSE - RETINT);//changes RETINT_ELSE -> RETINT and RETVOID_ELSE -> RETVOID
      }
      if(s == NTS_SEL_STMT && (t == ELSE_RETINT || t == ELSE_RETVOID))
      {
        t = t - (ELSE_RETINT - RETINT);//changes ELSE_RETINT -> RETINT and ELSE_RETVOID -> RETVOID
      }
      //printf("sym: %i = %i\n", s, NTS_RET_STMT);
      //printf("type: %i\n", t);
